...
```mermaid
graph TD
    A["📄 用户上传文档<br/>PDF / TXT / Markdown"] --> B["DocumentLoader<br/>解析 → 提取文本+元数据"]
    B --> C["TextSplitter<br/>递归分割 → Chunks (500字/块)"]
    C --> D["EmbeddingService<br/>sentence-transformers → 384维向量"]
    D --> E[("VectorStore<br/>ChromaDB<br/>持久化 + 余弦检索")]

    F["❓ 用户提问"] --> G["Retriever<br/>语义检索 Top-K 片段"]
    E --> G
    G --> H["Generator<br/>LLM 流式生成 (SSE)"]
    H --> I["💬 AI 回答<br/>含来源引用"]
```
